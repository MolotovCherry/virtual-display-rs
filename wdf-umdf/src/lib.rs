...
macro_rules! WdfFunction {
    ($name:ident, $($args:expr),*) => {{
        // wrapper function to re-wrap fn pointers in unsafe again, to force user to call unsafe
        // SAFETY: None
        //         User is responsible for all safety and must write their own unsafe block
        unsafe fn wdf_call() {
            let fn_handle = {
                $crate::paste! {
